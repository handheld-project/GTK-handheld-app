...
class Page2(Gtk.Grid):
    ...
    def confirm_action(self):
        if self.current_entity == 4 :
            ...
            self.on_go_back(self.goBackButton)
        elif self.current_entity == 6 :
            self.on_go_next(self.goNextButton)
        else :
            self.is_confirmed = True
    ...
    def on_go_back(self,widget) :
        try:
            ...
            GLib.idle_add(self.change_page_1)
    ...
    def prepare_for_page_change(self ,document):
        self.csvFileMaker.update_excel(document , self.exported_data['src_image'] )
        self.stop_event.set()
        GLib.idle_add(self.change_page_3)
    ...
    def on_go_next(self,widget) :
        ...
        if self.is_moveable or self.is_moveable is False:
            ...
            data_store_thread = threading.Thread(target=self.prepare_for_page_change(document))
    ...
    def get_data_from_hardware(self):
        try:
            with serial.Serial('/dev/ttyUSB0',9600) as ser :
                line = ser.readline().decode('utf8').strip()
                data_packed = {'valx':int(line[9:12]) , 'valy' : int(line[12:15]) , 'valz': int(line[15:18]) , 'longitude': float(line[20:29]) , 'latitude': float(line[29:38]) , 'distance': int(line[40:44])}
            return data_packed
    ...
    def set_data_from_hardware(self):
        ...
        data_pack = self.get_data_from_hardware()
        ...
        if data_pack :
            ...
            # latitude mock
            self.calculated_latitude.set_text(str(data_pack['longitude']))
            # longitude mock
            self.calculated_longitude.set_text(str(data_pack['longitude']))

        else :
            # w mock
            self.calculated_width.get_adjustment().set_value(0)
            # h mock
            self.calculated_height.get_adjustment().set_value(0)
            # latitude mock
            self.calculated_latitude.set_text("waiting..")
            # longitude mock
            self.calculated_longitude.set_text("waiting..")

        print("data_pack is :",data_pack)
